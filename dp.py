...
while(1):
    ...
    mask2 = cv2.inRange(hsv,np.array([2,50,50]),np.array([22,255,255]))
    ...
    for a in range(len(hull2)) :

        k = hull2[a]
       
        if cnts[k][0][0][1] < small :
            small = cnts[k][0][0][1]
            special = k
    ...
    if moments['m00']!=0:
        ...
        for i in range(0,len(hull)-1):
            if (np.absolute(hull[i][0][0] - hull[i+1][0][0]) > 10) or ( np.absolute(hull[i][0][1] - hull[i+1][0][1]) > 10):
                if hull[i][0][1] <500 :
        ...
        for i in range(0,len(fingers)):
            ...
            #than the distance of average finger webbing to center mass by 100 pixels
        result = 0
        for i in range(0,len(fingers)):
            if fingerDistance[i] > AverageDefectDistance+100:
                result = result +1
